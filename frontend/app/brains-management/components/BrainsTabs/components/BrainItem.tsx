...
export const BrainItem = ({ brain }: BrainItemProps): JSX.Element => {
  ...
  const { currentBrainId, setCurrentBrainId } = useBrainContext();

  const isBrainDescriptionEmpty = brain.description === "";
  const brainDescription = isBrainDescriptionEmpty
    ? t("empty_brain_description")
    : brain.description;
  const disableSetCurrentBrainId = brain.id === currentBrainId;

  const handleSetCurrentBrainId = () => {
    if (!disableSetCurrentBrainId) {
  ...
  return (
    <div className="flex justify-center items-center flex-col flex-1 w-full h-full shadow-md dark:shadow-primary/25 hover:shadow-xl transition-shadow rounded-xl overflow-hidden dark:bg-black border border-black/10 dark:border-white/25 pb-2 bg-secondary">
      ...
      <div className="w-full px-2 flex flex-col items-center gap-1">
        <button
          disabled={disableSetCurrentBrainId}
          onClick={() => handleSetCurrentBrainId()}
          className={`px-8 py-3 flex items-center justify-center gap-2 bg-white text-primary rounded-lg border-0 w-content mt-3 disabled:bg-vt-300 hover:bg-primary/50   disabled:cursor-no-drop w-full text-md`}
        >
          As current brain
        </button>
        <Link
